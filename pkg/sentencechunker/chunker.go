...
type Chunker struct {
	...
	// this chunker's tokenizer and size, so its windows obey the same
	// budget. Built in New, where its construction cannot fail: the same
	// size and tokenizer were just validated for hard.
...
func New(tok tokenizer.Tokenizer, size int, delims []string, opts ...Option) (Chunker, error) {
	...
	c := Chunker{tok: tok, size: size, delims: delims, hard: hard, fallback: fallback}
